...
function checkResults() {
    ...
    for (const line of lines) { //for each line that is read
        ...
        if ((parseInt(start1, 10) >= parseInt(start2, 10) && parseInt(end1, 10) <= parseInt(end2, 10)) || (parseInt(start2, 10) >= parseInt(start1, 10) && parseInt(end2, 10) <= parseInt(end1, 10))) fullOverlapCount +=1;

        if ((parseInt(end1, 10) >= parseInt(start2, 10) && parseInt(start1, 10) <= parseInt(start2, 10)) || (parseInt(end2, 10) >= parseInt(start1, 10) && parseInt(start2, 10) <= parseInt(start1, 10))) partialOverlapCount +=1;
    }
}
